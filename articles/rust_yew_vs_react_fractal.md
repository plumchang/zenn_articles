...
# （2026/9/4 追記： WASM SIMD で 高速化）

記事の公開後、[a-skua](https://github.com/a-skua) さんから Yew 版のリポジトリに [PR](https://github.com/plumchang/yew-fractal/pull/1) をいただきました。ありがとうございました！🙌  
（なお、PR を出していただいのが 6月 だったので、だいぶ間を空けてしまいました。。。たいへん申し訳ございませんでした。）

> SIMD使ったらさらに高速化できるんじゃない?ということで試してみたもの

ありがたく取り込ませていただいたので、本記事にも追記いたします。

## WASM SIMD とは

WebAssembly には **`v128`** という 128 ビットの SIMD 型があります。SIMD（Single Instruction, Multiple Data）は「1 つの命令で複数のデータを同時に処理する」仕組みで、128 ビットあれば `f32` なら 4 個、`f64` なら 2 個を 1 レーンずつ詰めて並列に計算できます。

JavaScript にはこれを明示的に使う手段が無いようで、**WASM 固有の武器**ということになります。

## 注意点：対応ブラウザ

SIMD 命令に非対応していないバージョンのブラウザでは、アプリが動きません。
...
## 実装：`f32x4` で 4 点を同時に計算

まず、`wasm32-unknown-unknown` ターゲットに `simd128` を有効化します。
...
そのうえで、4 点分の複素数を受け取って反復回数を 4 つ返す関数を追加します。実装は[リポジトリ](https://github.com/plumchang/yew-fractal/blob/main/src/fractal.rs#L88-L167)を参照してください。
...
| シーン | Frame |
|---|---|
| Zoom 1.9x（初期画面） | **7.4 ms** |
| Zoom 約 380x（中ズーム） | **28.1 ms** |
| Zoom 約 72,000x（深ズーム） | **26.0 ms** |

:::message
本編の計測時とは、画面サイズなどの計測環境が揃っていないため、目安になります。
:::

**初期画面Frameを 1 としたときの倍率**で比べると、

| | 本編（SIMD 前） | 今回（SIMD 後） |
|---|---|---|
| Zoom 約 380x | 15.7 倍 | **3.8 倍** |
| Zoom 約 72,000x | 15.3 倍 | **3.5 倍** |

ズーム時の倍率が下がっており、**重い計算の部分に SIMD がしっかり効いている**と考えられます。  
実際に操作した体感としても、カクツキがかなり減ったように感じました。

## 所感
まず、公開後の記事・サンプルコードに対してフィードバックをいただけたのが、とても嬉しい体験でした。  
SIMDという「WASM の優位性」があることも知れて、非常に勉強になりました。  
a-skua さん、ありがとうございました！
